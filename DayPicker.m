...
- (id)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        self.days = [[NSMutableDictionary alloc]
                     initWithObjectsAndKeys:[NSNumber numberWithInt:0], @"monday",
                     [NSNumber numberWithInt:0], @"tuesday",
                     [NSNumber numberWithInt:0], @"wednesdaynesday",
                     [NSNumber numberWithInt:0], @"thursday",
                     [NSNumber numberWithInt:0], @"friday",
                     [NSNumber numberWithInt:0], @"saturday",
                     [NSNumber numberWithInt:0], @"sunday", nil];
        
        sunday = [[DayPickerButton alloc]
                  initWithFrame:CGRectMake(0, 5, self.frame.size.width / 7,
                                           self.frame.size.height - 10)
                  andTitle:@"S"];
        [sunday addTextColor:tc andHighlightTextColor:htc];
        [sunday addHighlightColor:hl];
        [sunday addTarget:self
                   action:@selector(sunday:)
         forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:sunday];
        
        monday = [[DayPickerButton alloc]
                  initWithFrame:CGRectMake(self.frame.size.width / 7, 5,
                                           self.frame.size.width / 7,
                                           self.frame.size.height - 10)
                  andTitle:@"M"];
        [monday addTextColor:tc andHighlightTextColor:htc];
        [monday addHighlightColor:hl];
        [monday addTarget:self
                   action:@selector(monday:)
         forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:monday];
        
        tuesday = [[DayPickerButton alloc]
                   initWithFrame:CGRectMake(2 * self.frame.size.width / 7, 5,
                                            self.frame.size.width / 7,
                                            self.frame.size.height - 10)
                   andTitle:@"T"];
        [tuesday addTextColor:tc andHighlightTextColor:htc];
        [tuesday addHighlightColor:hl];
        [tuesday addTarget:self
                    action:@selector(tuesday:)
          forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:tuesday];
        
        wednesday = [[DayPickerButton alloc]
                     initWithFrame:CGRectMake(3 * self.frame.size.width / 7, 5,
                                              self.frame.size.width / 7,
                                              self.frame.size.height - 10)
                     andTitle:@"W"];
        [wednesday addTextColor:tc andHighlightTextColor:htc];
        [wednesday addHighlightColor:hl];
        [wednesday addTarget:self
                      action:@selector(wednesday:)
            forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:wednesday];
        
        thursday = [[DayPickerButton alloc]
                    initWithFrame:CGRectMake(4 * self.frame.size.width / 7, 5,
                                             self.frame.size.width / 7,
                                             self.frame.size.height - 10)
                    andTitle:@"Th"];
        [thursday addTextColor:tc andHighlightTextColor:htc];
        [thursday addHighlightColor:hl];
        [thursday addTarget:self
                     action:@selector(thursday:)
           forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:thursday];
        
        friday = [[DayPickerButton alloc]
                  initWithFrame:CGRectMake(5 * self.frame.size.width / 7, 5,
                                           self.frame.size.width / 7,
                                           self.frame.size.height - 10)
                  andTitle:@"F"];
        [friday addTextColor:tc andHighlightTextColor:htc];
        [friday addHighlightColor:hl];
        [friday addTarget:self
                   action:@selector(friday:)
         forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:friday];
        
        saturday = [[DayPickerButton alloc]
                    initWithFrame:CGRectMake(6 * self.frame.size.width / 7, 5,
                                             self.frame.size.width / 7,
                                             self.frame.size.height - 10)
                    andTitle:@"S"];
        [saturday addTextColor:tc andHighlightTextColor:htc];
        [saturday addHighlightColor:hl];
        [saturday addTarget:self
                     action:@selector(saturday:)
           forControlEvents:UIControlEventTouchUpInside];

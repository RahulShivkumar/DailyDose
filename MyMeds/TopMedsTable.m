...
#define kSizeOfView (self.frame.size.height - 27) * MAX(1, [[self.data allKeys] count]) / 5.0
...
- (void)setupViews{
    title = [[UILabel alloc] initWithFrame:CGRectMake(17, 0, self.frame.size.width, 20)];
    [title setText:@"TOP MISSED MEDS (4 WEEKS):"];
    [title setTextColor:[UIColor lightGrayColor]];
    [title setFont:[UIFont systemFontOfSize:12]];
    [self addSubview:title];
    
    bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 27, self.frame.size.width, kSizeOfView)];
...
- (void)addMedToView:(int)index andYPosition:(float)y {
    ...
    UILabel *medLabel = [[UILabel alloc] initWithFrame:CGRectMake(17, y, bgView.frame.size.width * 0.70, 20)];
    [medLabel setText:key];
    [bgView addSubview:medLabel];
    
    UILabel *valueLabel = [[UILabel alloc] initWithFrame:CGRectMake(bgView.frame.size.width * 0.80, y, bgView.frame.size.width * 0.20, 20)];
    [valueLabel setText:[NSString stringWithFormat:@"%@",[self.data objectForKey:key]]];
    [bgView addSubview:valueLabel];
}

- (void)addNoDataAvailable:(float)y {
    
    UILabel *noDataLabel = [[UILabel alloc] initWithFrame:CGRectMake(17, y, bgView.frame.size.width * 0.70, 20)];
...
- (void)addLineSeparator:(float)y {
    UIView *lineSeperator1 = [[UIView alloc] initWithFrame:CGRectMake(0, y, bgView.frame.size.width, 0.5)];

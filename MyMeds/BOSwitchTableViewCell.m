...
- (void)setup {
    ...
    [self.toggleSwitch addTarget:self
                          action:@selector(toggleSwitchValueDidChange)
    ...
    [self.toggleSwitch.superview
     addConstraints:@[ centerYConstraint, rightConstraint ]];
}
...
- (NSString *)footerTitle {
    if (self.onFooterTitle || self.offFooterTitle) {
        return [self.setting.value boolValue] ? self.onFooterTitle
        : self.offFooterTitle;
    }
...
- (void)settingValueDidChange {
    [self.toggleSwitch setOn:[self.setting.value boolValue]

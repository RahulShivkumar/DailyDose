...
- (void)awakeFromNib {
    ...
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc]
                                                    initWithTarget:self
                                                    action:@selector(tableViewReceivedTap)];
        ...
        [[self.navigationController transitionCoordinator]
         notifyWhenInteractionEndsUsingBlock:^(id<
             ...
             if ([context isCancelled]) {
                 [self.tableView selectRowAtIndexPath:selectedRowIndexPath
                                             animated:YES
...
willDisplayHeaderView:(UIView *)view
    ...
    if (self.headerTitlesColor)
        header.textLabel.textColor = self.headerTitlesColor;
    if (self.headerTitlesFont)
        header.textLabel.font = self.headerTitlesFont;
}

- (CGFloat)tableView:(UITableView *)tableView
heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat rowHeight =
    MAX(self.tableView.rowHeight,
        [super tableView:tableView heightForRowAtIndexPath:indexPath]);
    
    BOTableViewCell *cell = (BOTableViewCell *)
    [self tableView:tableView cellForRowAtIndexPath:indexPath];
    if ([cell isKindOfClass:[BOTableViewCell class]]) {
        return rowHeight + ([indexPath isEqual:self.expansionIndexPath]
...
- (void)tableView:(UITableView *)tableView
  willDisplayCell:(BOTableViewCell *)cell
forRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[BOTableViewCell class]]) {
        ...
        if (cell.layoutMargins.top == 8) {
            ...
            cell.layoutMargins =
            UIEdgeInsetsMake(cellHeight, cell.layoutMargins.left,
                             cell.layoutMargins.bottom, cell.layoutMargins.right);
        }
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    BOTableViewCell *cell =
    (BOTableViewCell *)[tableView cellForRowAtIndexPath:indexPath];
    
    if (cell.expansionHeight > 0) {
        self.expansionIndexPath = ![cell.indexPath isEqual:self.expansionIndexPath]
        ? cell.indexPath
        : nil;
    ...
    if ([cell respondsToSelector:@selector(wasSelectedFromViewController:)])
...
titleForFooterInSection:(NSInteger)section {
    // First, we get the super value, and if it's nil we set it to an empty string
    // (this is the lowest priority for dynamic footers).
    NSString *footerTitle =
    [super tableView:self.tableView titleForFooterInSection:section];
    if (!footerTitle)
        footerTitle = @"";
    
    // Next, we try to find an existing footer in the last cell of the section
    // (this is the medium priority for dynamic footers).
    NSInteger numberOfRows =
    [super tableView:self.tableView numberOfRowsInSection:section];
    BOTableViewCell *lastCell = (BOTableViewCell *)
    [super tableView:self.tableView
cellForRowAtIndexPath:[NSIndexPath indexPathForRow:numberOfRows - 1
                                         inSection:section]];
    if ([lastCell isKindOfClass:[BOTableViewCell class]] &&
        [lastCell footerTitle])
        footerTitle = [lastCell footerTitle];
    
    // Finally, we try to find an existing footer in any cell that has a checkmark
    // accessory on it (this is the top priority for dynamic footers).
...
willDisplayFooterView:(UIView *)view
    ...
    if (self.footerTitlesColor)
        footer.textLabel.textColor = self.footerTitlesColor;
    if (self.footerTitlesFont)
        footer.textLabel.font = self.footerTitlesFont;
}

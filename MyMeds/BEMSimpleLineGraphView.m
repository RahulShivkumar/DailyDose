...
#pragma clang diagnostic pop
                         ...
                         if (numberOfGaps >= (numberOfPoints - 1)) {
                             ...
                             @autoreleasepool {
                                 NSInteger offset =
                                 [self offsetForXAxisWithNumberOfGaps:numberOfGaps]; // The offset
                                 // (if possible
                                 // and
                                 // necessary)
                                 // used to shift
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
    ...
    if (self.positionYAxisRight) {
        ...
        positionOnXAxis = (((self.frame.size.width - self.YAxisLabelXOffset) /
                            (numberOfPoints - 1)) *
                           index) +
        self.YAxisLabelXOffset + horizontalTranslation;
    }
    
    // Set the final center point of the x-axis labels
    center = CGPointMake(positionOnXAxis,
                         self.frame.size.height - lRect.size.height + 5);
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            NSString *formattedValue = [NSString
                                        stringWithFormat:self.formatStringForValues, dotValue.doubleValue];
        ...
        for (NSInteger i = numberOfLabels; i > 0; i--) {
            ...
            labelYAxis.center = CGPointMake(xValueForCenterLabelYAxis, yAxisPosition);
            labelYAxis.text =
            [NSString stringWithFormat:self.formatStringForValues,
             (graphHeight - self.XAxisLabelYOffset -
              yAxisPosition)];
            labelYAxis.font = self.labelFont;
            labelYAxis.textAlignment = textAlignmentForLabelYAxis;
            labelYAxis.textColor = self.colorYaxisLabel;
...
- (NSNumber *)calculatePointValueAverage {
    ...
    NSExpression *expression = [NSExpression
                                ...
                                arguments:@[
                                            [NSExpression expressionForConstantValue:filteredArray]
                                            ]];

...
didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (!UUID) {
        // If the app is deleted, we want to remove exisiting local notifications
        // when a user re downloads
        [[UIApplication sharedApplication] cancelAllLocalNotifications];
        [[Amplitude instance] initializeApiKey:@"6173840dbe787b3041206d97de06b633"
...
- (NSString *)getUserID {
    ...
    NSString *uuidStr = (__bridge_transfer NSString *)CFUUIDCreateString(
                                                                         kCFAllocatorDefault, uuidObject);
...
forLocalNotification:(UILocalNotification *)notification
  completionHandler:(void (^)())completionHandler {
    ...
    if ([identifier isEqualToString:@"taken"]) {
        DBResultSet *tms =
        [[[TodayMedication query] whereWithFormat:@"time = %f", time] fetch];

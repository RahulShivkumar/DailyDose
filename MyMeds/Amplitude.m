...
+ (void)logEvent:(NSString *)eventType
withEventProperties:(NSDictionary *)eventProperties {
...
+ (void)logRevenue:(NSString *)productIdentifier
          quantity:(NSInteger)quantity
             price:(NSNumber *)price {
    [[Amplitude instance] logRevenue:productIdentifier
                            quantity:quantity
                               price:price];
}
...
- (id)init {
    if (self = [super init]) {
        ...
        // Force method calls to happen in FIFO order by only allowing 1 concurrent
        // operation
        [_backgroundQueue setMaxConcurrentOperationCount:1];
        // Ensure initialize finishes running asynchronously before other calls are
        // run
        [_backgroundQueue setSuspended:YES];
        // Name the queue so runOnBackgroundQueue can tell which queue an operation
        // is running
        ...
        [_initializerQueue addOperationWithBlock:^{
            ...
            NSString *eventsDataDirectory = [NSSearchPathForDirectoriesInDomains(
                                                                                 NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0];
            
            _propertyListPath = SAFE_ARC_RETAIN([eventsDataDirectory
                                                 stringByAppendingPathComponent:@"com.amplitude.plist"]);
            _eventsDataPath = SAFE_ARC_RETAIN([eventsDataDirectory
                                               stringByAppendingPathComponent:@"com.amplitude.archiveDict"]);
            
            // Copy any old data files to new file paths
            NSString *oldEventsDataDirectory = [NSSearchPathForDirectoriesInDomains(
                                                                                    NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
            NSString *oldPropertyListPath = [oldEventsDataDirectory
                                             stringByAppendingPathComponent:@"com.amplitude.plist"];
            NSString *oldEventsDataPath = [oldEventsDataDirectory
                                           stringByAppendingPathComponent:@"com.amplitude.archiveDict"];
            ...
            _propertyList =
            SAFE_ARC_RETAIN([self deserializePList:_propertyListPath]);
            if (!_propertyList) {
                _propertyList = SAFE_ARC_RETAIN([NSMutableDictionary dictionary]);
                [_propertyList setObject:[NSNumber numberWithLongLong:0LL]
                                  forKey:@"max_id"];
                BOOL success = [self savePropertyList];
                if (!success) {
                    NSLog(
                          @"ERROR: Unable to save propertyList to file on initialization");
                }
            } else {
            ...
            if (!_eventsData) {
                ...
                [_eventsData setObject:[NSNumber numberWithLongLong:0LL]
                                forKey:@"max_id"];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [_locationManager performSelector:setDelegate
                                   withObject:_locationManagerDelegate];
        });
...
- (void)initializeApiKey:(NSString *)apiKey userId:(NSString *)userId {
    ...
    [self initializeApiKey:apiKey
                    userId:userId
              startSession:(state != UIApplicationStateBackground)];
}

/**
 * Initialize Amplitude with a given apiKey and userId. Optionally, start a
 * session at the same time.
 */
- (void)initializeApiKey:(NSString *)apiKey
                  userId:(NSString *)userId
            startSession:(BOOL)startSession {
    if (apiKey == nil) {
        NSLog(@"ERROR: apiKey cannot be nil in initializeApiKey:");
        return;
    }
    
    if (![self isArgument:apiKey
                validType:[NSString class]
               methodName:@"initializeApiKey:"]) {
        return;
    }
    if (userId != nil &&
        ![self isArgument:userId
                validType:[NSString class]
               methodName:@"initializeApiKey:"]) {
            return;
        }
...
withEventProperties:(NSDictionary *)eventProperties {
    if (![self isArgument:eventType
                validType:[NSString class]
               methodName:@"logEvent:withEventProperties:"]) {
        return;
    }
    if (eventProperties != nil &&
        ![self isArgument:eventProperties
                validType:[NSDictionary class]
               methodName:@"logEvent:withEventProperties:"]) {
            return;
        }
...
- (void)logEvent:(NSString *)eventType
withEventProperties:(NSDictionary *)eventProperties
   apiProperties:(NSDictionary *)apiProperties
   withTimestamp:(NSNumber *)timestamp {
    if (_apiKey == nil) {
        NSLog(@"ERROR: apiKey cannot be nil or empty, set apiKey with "
              @"initializeApiKey: before calling logEvent:");
        return;
    }
    
    if (timestamp == nil) {
        timestamp = [NSNumber
                     numberWithLongLong:[[NSDate date] timeIntervalSince1970] * 1000];
    }
    ...
    [self runOnBackgroundQueue:^{
        ...
        @synchronized(_eventsData) {
            ...
            if ([[_eventsData objectForKey:@"opt_out"] boolValue]) {
                NSLog(@"User has opted out of tracking. Event %@ not logged.",
                      eventType);
                return;
            }
            
            // Increment propertyList max_id and save immediately
            NSNumber *propertyListMaxId = [NSNumber
                                           numberWithLongLong:
                                           [[_propertyList objectForKey:@"max_id"] longLongValue] + 1];
            [_propertyList setObject:propertyListMaxId forKey:@"max_id"];
            [self savePropertyList];
            
            // Increment _eventsData max_id
            long long newId =
            [[_eventsData objectForKey:@"max_id"] longLongValue] + 1;
            
            [event setValue:eventType forKey:@"event_type"];
            [event setValue:[NSNumber numberWithLongLong:newId] forKey:@"event_id"];
            [event setValue:[self replaceWithEmptyJSON:eventProperties]
                     forKey:@"event_properties"];
            [event setValue:[self replaceWithEmptyJSON:apiProperties]
                     forKey:@"api_properties"];
            [event setValue:[self replaceWithEmptyJSON:_userProperties]
                     forKey:@"user_properties"];
            
            [self addBoilerplate:event
                       timestamp:timestamp
                      maxIdCheck:propertyListMaxId];
            [self refreshSessionTime:timestamp];
            
            [[_eventsData objectForKey:@"events"] addObject:event];
            [_eventsData setObject:[NSNumber numberWithLongLong:newId]
                            forKey:@"max_id"];
            
            AMPLITUDE_LOG(@"Logged %@ Event", event[@"event_type"]);
            
            if ([[_eventsData objectForKey:@"events"] count] >= kAMPEventMaxCount) {
                // Delete old events if list starting to become too large to comfortably
                // work with in memory
                [[_eventsData objectForKey:@"events"]
                 removeObjectsInRange:NSMakeRange(0, kAMPEventRemoveBatchSize)];
                [self saveEventsData];
            } else if ([[_eventsData objectForKey:@"events"] count] >=
                       kAMPEventRemoveBatchSize &&
                       [[_eventsData objectForKey:@"events"] count] %
                       kAMPEventRemoveBatchSize ==
                       0) {
                [self saveEventsData];
            }
            
            if ([[_eventsData objectForKey:@"events"] count] >=
                kAMPEventUploadThreshold) {
                [self uploadEvents];
            } else {
...
- (void)addBoilerplate:(NSMutableDictionary *)event
    ...
    NSDictionary *library = @{ @"name" : kAMPLibrary, @"version" : kAMPVersion };
    ...
    if (_lastKnownLocation != nil) {
        @synchronized(_locationManager) {
            ...
            NSMethodSignature *coordinateMethodSignature =
            [_lastKnownLocation methodSignatureForSelector:coordinateSelector];
            NSInvocation *coordinateInvocation = [NSInvocation
                                                  invocationWithMethodSignature:coordinateMethodSignature];
            ...
            [location setValue:[NSNumber numberWithDouble:lastKnownLocationCoordinate
                                .latitude]
                        forKey:@"lat"];
            [location setValue:[NSNumber numberWithDouble:lastKnownLocationCoordinate
                                .longitude]
...
- (void)logRevenue:(NSString *)productIdentifier
    ...
    if (_apiKey == nil) {
        NSLog(@"ERROR: apiKey cannot be nil or empty, set apiKey with "
              @"initializeApiKey: before calling logRevenue:");
        return;
    }
    if (![self isArgument:price
                validType:[NSNumber class]
               methodName:@"logRevenue:"]) {
        return;
    }
    ...
    [apiProperties setValue:[NSNumber numberWithInteger:quantity]
                     forKey:@"quantity"];
    [apiProperties setValue:price forKey:@"price"];
    
    if ([receipt respondsToSelector:@selector(base64EncodedStringWithOptions:)]) {
        [apiProperties setValue:[receipt base64EncodedStringWithOptions:0]
                         forKey:@"receipt"];
    } else {
#pragma clang diagnostic push
...
- (void)uploadEventsWithDelay:(int)delay {
    if (!_updateScheduled) {
        ...
        [_mainQueue addOperationWithBlock:^{
            [self performSelector:@selector(uploadEventsExecute)
                       withObject:nil
                       afterDelay:delay];
        }];
    }
...
- (void)uploadEventsWithLimit:(int)limit {
    if (_apiKey == nil) {
        NSLog(@"ERROR: apiKey cannot be nil or empty, set apiKey with "
              @"initializeApiKey: before calling uploadEvents:");
        return;
    }
    ...
    [self runOnBackgroundQueue:^{
        ...
        @synchronized(_eventsData) {
            ...
            long long numEvents =
            limit ? fminl([events count], limit) : [events count];
            if (numEvents == 0) {
                _updatingCurrently = NO;
                return;
            }
            NSArray *uploadEvents =
            [events subarrayWithRange:NSMakeRange(0, (int)numEvents)];
            long long lastEventIDUploaded =
            [[[uploadEvents lastObject] objectForKey:@"event_id"] longLongValue];
            NSError *error = nil;
            NSData *eventsDataLocal = nil;
            @try {
                eventsDataLocal = [NSJSONSerialization
                                   dataWithJSONObject:[self makeJSONSerializable:uploadEvents]
                                   options:0
                                   error:&error];
            ...
            if (eventsDataLocal) {
                NSString *eventsString = SAFE_ARC_AUTORELEASE(
                                                              [[NSString alloc] initWithData:eventsDataLocal
                                                                                    encoding:NSUTF8StringEncoding]);
                [self makeEventUploadPostRequest:kAMPEventLogUrl
                                          events:eventsString
                             lastEventIDUploaded:lastEventIDUploaded];
            }
        }
...
- (void)makeEventUploadPostRequest:(NSString *)url
                            events:(NSString *)events
               lastEventIDUploaded:(long long)lastEventIDUploaded {
    ...
    [postData
     appendData:[apiVersionString dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    [postData appendData:[[self urlEncodeString:events]
                          dataUsingEncoding:NSUTF8StringEncoding]];
    
    // Add timestamp of upload
    [postData
     appendData:[@"&upload_time=" dataUsingEncoding:NSUTF8StringEncoding]];
    NSString *timestampString =
    [[NSNumber numberWithLongLong:[[NSDate date] timeIntervalSince1970] *
      1000] stringValue];
    [postData
     appendData:[timestampString dataUsingEncoding:NSUTF8StringEncoding]];
    
    // Add checksum
    [postData appendData:[@"&checksum=" dataUsingEncoding:NSUTF8StringEncoding]];
    NSString *checksumData =
    [NSString stringWithFormat:@"%@%@%@%@", apiVersionString, _apiKey, events,
     timestampString];
...
#if AMPLITUDE_SSL_PINNING
    id Connection = (self.sslPinningEnabled ? [AMPURLConnection class]
...
#endif
      {
                  ...
                  if ([result isEqualToString:@"success"]) {
                      ...
                      @synchronized(_eventsData) {
                          ...
                          for (id event in
                              ...
                              i++;
                              if ([[event
                                    objectForKey:@"event_id"] longLongValue] ==
                                  lastEventIDUploaded) {
                                  numberToRemove = i;
                                  break;
                              }
                          ...
                          [[_eventsData objectForKey:@"events"]
                           removeObjectsInRange:NSMakeRange(
                                                            0, (int)numberToRemove)];
                      }
                  } else if ([result isEqualToString:@"invalid_api_key"]) {
                      NSLog(@"ERROR: Invalid API Key, make sure your API key "
                            @"is correct in initializeApiKey:");
                  } else if ([result isEqualToString:@"bad_checksum"]) {
                      NSLog(@"ERROR: Bad checksum, post request was mangled "
                            @"in transit, will attempt to reupload later");
                  } else if ([result isEqualToString:
                              @"request_db_write_failed"]) {
                      NSLog(@"ERROR: Couldn't write to request database on "
                            @"server, will attempt to reupload later");
                  } else {
                      NSLog(@"ERROR: %@, will attempt to reupload later",
                            result);
                  }
                  SAFE_ARC_RELEASE(result);
              } else {
                  NSLog(@"ERROR: Connection response received:%ld, %@",
                        (long)[httpResponse statusCode],
                        SAFE_ARC_AUTORELEASE([[NSString alloc]
                                              initWithData:data
                                              encoding:NSUTF8StringEncoding]));
              }
          } else if (error != nil) {
              if ([error code] == -1009) {
                  AMPLITUDE_LOG(@"No internet connection (not connected to "
                                @"internet), unable to upload events");
              } else if ([error code] == -1003) {
                  AMPLITUDE_LOG(@"No internet connection (hostname not "
                                @"found), unable to upload events");
              } else if ([error code] == -1001) {
                  AMPLITUDE_LOG(@"No internet connection (request timed "
                                @"out), unable to upload events");
              } else {
                  NSLog(@"ERROR: Connection error:%@", error);
              }
          } else {
              NSLog(@"ERROR: response empty, error empty for "
                    @"NSURLConnection");
          }
          ...
          if (uploadSuccessful &&
              ...
              [[UIApplication sharedApplication]
               endBackgroundTask:_uploadTaskID];
              _uploadTaskID = UIBackgroundTaskInvalid;
          }
      }];
     }
...
#pragma mark - application lifecycle methods
     ...
     - (void)enterBackground {
         ...
         _uploadTaskID = [[UIApplication sharedApplication]
...
#pragma mark - Sessions
     ...
     - (void)startSession {
         ...
         [self runOnBackgroundQueue:^{
             @synchronized(_eventsData) {
                 ...
                 NSNumber *previousSessionTime =
                 [_eventsData objectForKey:@"previous_session_time"];
                 long long timeDelta =
                 [now longLongValue] - [previousSessionTime longLongValue];
                 ...
                 if (!sessionExists && timeDelta < kAMPMinTimeBetweenSessionsMillis) {
                     // The previous session happened recently enough to be considered the
                     // same session. Use it.
                     _sessionId =
                     [[_eventsData objectForKey:@"previous_session_id"] longLongValue];
                 } else if (!sessionExists || sessionExpired) {
                     // Session has expired or doesn't exist.
                     _sessionId = [now longLongValue];
                     [_eventsData setValue:[NSNumber numberWithLongLong:_sessionId]
                                    forKey:@"previous_session_id"];
                 }
    ...
    withEventProperties:nil
         ...
         [_mainQueue addOperationWithBlock:^{
             [self performSelector:@selector(turnOffSessionLaterExecute)
                        withObject:nil
                        afterDelay:kAMPMinTimeBetweenSessionsMillis];
         }];
     }
...
#pragma mark - configurations
     ...
     - (void)setUserProperties:(NSDictionary *)userProperties replace:(BOOL)replace {
         if (![self isArgument:userProperties
                     validType:[NSDictionary class]
     ...
     - (void)setOptOut:(BOOL)enabled {
         [self runOnBackgroundQueue:^{
             @synchronized(_eventsData) {
                 [_eventsData setObject:[NSNumber numberWithBool:enabled]
                                 forKey:@"opt_out"];
                 [self saveEventsData];
             }
         }];
...
#pragma mark - Getters for device data
     ...
     - (id)makeJSONSerializable:(id)obj {
         ...
         if ([obj isKindOfClass:[NSDictionary class]]) {
             ...
             for (id key in obj) {
                 ...
                 if (![key isKindOfClass:[NSString class]]) {
                     ...
                     NSLog(@"WARNING: Non-string property key, received %@, coercing to %@",
                           [key class], coercedKey);
                 } else {
                     coercedKey = key;
                 }
         ...
         NSLog(@"WARNING: Invalid property value type, received %@, coercing to %@",
               [obj class], str);
         return str;
     }
     
     - (BOOL)isArgument:(id)argument
     validType:(Class) class
     methodName:(NSString *)methodName {
         if ([argument isKindOfClass:class]) {
             return YES;
         } else {
             NSLog(@"ERROR: Invalid type argument to method %@, expected %@, received "
                   @"%@, ",
                   methodName, class, [argument class]);
             return NO;
         }
     }
...
#else
         newString = NSMakeCollectable(CFURLCreateStringByAddingPercentEscapes(
     ...
     - (void)printEventsCount {
         NSLog(@"Events count:%ld",
               (long)[[_eventsData objectForKey:@"events"] count]);
     }

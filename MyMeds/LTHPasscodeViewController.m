...
#define kPasscodeCharWidth                                                     \
[_passcodeCharacter                                                          \
sizeWithAttributes:@{NSFontAttributeName : _passcodeFont}].width
#define kFailedAttemptLabelWidth                                               \
(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad                        \
? [_failedAttemptLabel.text                                             \
sizeWithAttributes:@{NSFontAttributeName : _labelFont}].width +   \
60.0f                                                             \
: [_failedAttemptLabel.text                                             \
sizeWithAttributes:@{NSFontAttributeName : _labelFont}].width +   \
30.0f)
...
[_failedAttemptLabel.text                                                    \
sizeWithAttributes:@{NSFontAttributeName : _labelFont}].height
#define kEnterPasscodeLabelWidth                                               \
[_enterPasscodeLabel.text                                                    \
sizeWithAttributes:@{NSFontAttributeName : _labelFont}].width
#else
...
#define kFailedAttemptLabelWidth                                               \
(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad                        \
? [_failedAttemptLabel.text sizeWithFont:_labelFont].width + 60.0f      \
: [_failedAttemptLabel.text sizeWithFont:_labelFont].width + 20.0f)
#define kFailedAttemptLabelHeight                                              \
[_failedAttemptLabel.text sizeWithFont:_labelFont].height
#define kEnterPasscodeLabelWidth                                               \
[_enterPasscodeLabel.text sizeWithFont:_labelFont].width
#endif
...
BOOL isUserSwitchingBetweenPasscodeModes; // simple/complex
...
- (void)_saveTimerStartTime {
    ...
    [LTHKeychainUtils
     storeUsername:_keychainTimerStartUsername
     andPassword:
    ...
    if (!self.context && _allowUnlockWithTouchID) {
        ...
        if ([self.context
            ...
            [self.context
             ...
             localizedReason:NSLocalizedStringFromTable(
                     ...
                     dispatch_async(dispatch_get_main_queue(), ^{
                         ...
                         if ([self.delegate
                              ...
                              @selector(
                                  ...
                                  [self.delegate
                                   performSelector:
                                   @selector(passcodeWasEnteredSuccessfully)];
                              }
                     });
                 } else {
                     dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_passcodeTextField.isFirstResponder &&
        (!_isUsingTouchID || _isUserChangingPasscode ||
         _isUserBeingAskedForNewPasscode || _isUserConfirmingPasscode ||
         _isUserEnablingPasscode || _isUserSwitchingBetweenPasscodeModes ||
         _isUserTurningPasscodeOff)) {
            [_passcodeTextField becomeFirstResponder];
            _animatingView.hidden = NO;
        }
    if (_isUsingTouchID && !_isUserChangingPasscode &&
        !_isUserBeingAskedForNewPasscode && !_isUserConfirmingPasscode &&
        !_isUserEnablingPasscode && !_isUserSwitchingBetweenPasscodeModes &&
        !_isUserTurningPasscodeOff) {
        [_passcodeTextField resignFirstResponder];
...
- (void)_dismissMe {
    ...
    [self _resetUI];
    [_passcodeTextField resignFirstResponder];
    [UIView animateWithDuration:_lockAnimationDuration
                     animations:^{
                         if (_displayedAsLockScreen) {
                             if (LTHiOS8) {
                                 self.view.center =
                                 CGPointMake(self.view.center.x, self.view.center.y * 2.f);
                             } else {
                                 if ([UIApplication sharedApplication].statusBarOrientation ==
                                     UIInterfaceOrientationLandscapeLeft) {
                                     self.view.center =
                                     CGPointMake(self.view.center.x * -1.f, self.view.center.y);
                                 } else if ([UIApplication sharedApplication].statusBarOrientation ==
                                            UIInterfaceOrientationLandscapeRight) {
                                     self.view.center =
                                     CGPointMake(self.view.center.x * 2.f, self.view.center.y);
                                 } else if ([UIApplication sharedApplication].statusBarOrientation ==
                                            UIInterfaceOrientationPortrait) {
                                     self.view.center =
                                     CGPointMake(self.view.center.x, self.view.center.y * -1.f);
                                 } else {
                                     self.view.center =
                                     CGPointMake(self.view.center.x, self.view.center.y * 2.f);
                                 }
                     ...
                     completion:^(BOOL finished) {
                         if ([self.delegate
                              respondsToSelector:@selector(
                                                           passcodeViewControllerWillClose)]) {
                                  [self.delegate
                                   performSelector:@selector(passcodeViewControllerWillClose)];
                              }
                         // Or, if you prefer by notifications:
                         //		[[NSNotificationCenter defaultCenter]
                         // postNotificationName: @"passcodeViewControllerWillClose"
                         //															object:
                         // self
                         //														  userInfo:
                         // nil];
...
- (void)_setupLabels {
    ...
    _enterPasscodeLabel.text =
    _isUserChangingPasscode
    ? NSLocalizedStringFromTable(self.enterOldPasscodeString,
                                 _localizationTableName, @"")
    : NSLocalizedStringFromTable(self.enterPasscodeString,
                                 _localizationTableName, @"");
...
- (void)_setupOKButton {
    ...
    [_OKButton
     setTitle:NSLocalizedStringFromTable(@"OK", _localizationTableName, nil)
    ...
    [_OKButton setTitleColor:[UIColor blackColor]
...
- (void)updateViewConstraints {
    ...
    // MARK: Please read
    // The controller works properly on all devices and orientations, but looks
    // odd on iPhone's landscape.
    // Usually, lockscreens on iPhone are kept portrait-only, though. It also
    // doesn't fit inside a modal when landscape.
    // That's why only portrait is selected for iPhone's supported orientations.
    // Modify this to fit your needs.
    
    CGFloat yOffsetFromCenter =
    -self.view.frame.size.height * 0.24 + _verticalOffset;
    ...
    if (self.isSimple) {
        ...
        NSDictionaryOfVariableBindings(_passcodeTextField, _OKButton);
        
        // TODO: specify different offsets through metrics
        NSArray *constraints = [NSLayoutConstraint
                                constraintsWithVisualFormat:
                                @"H:|-10-[_passcodeTextField]-5-[_OKButton]-10-|"
                                options:0
                                metrics:nil
        ...
        constraints = [NSLayoutConstraint
                       constraintsWithVisualFormat:@"V:|-5-[_passcodeTextField]-5-|"
                       options:0
                       metrics:nil
                       views:viewsDictionary];
...
- (void)showLockScreenWithAnimation:(BOOL)animated
                         withLogout:(BOOL)hasLogout
                     andLogoutTitle:(NSString *)logoutTitle {
    [self _prepareAsLockScreen];
    
    // In case the user leaves the app while the lockscreen is already active.
    if (!_isCurrentlyOnScreen) {
        // Usually, the app's window is the first on the stack. I'm doing this
        // because if an alertView or actionSheet
        // is open when presenting the lockscreen causes problems, because the av/as
        // has it's own window that replaces the keyWindow
        // and due to how Apple handles said window internally.
        // Currently the lockscreen appears behind the av/as, which is the best
        // compromise for now.
        ...
        // Usually not more than one window is needed, but your needs may vary;
        // modify below.
        ...
        // All this hassle because a view added to UIWindow does not rotate
        // automatically
        // and if we would have added the view anywhere else, it wouldn't display
        // properly
        ...
        if (hasLogout) {
            ...
            self.navBar = [[UINavigationBar alloc]
                           initWithFrame:CGRectMake(0, mainWindow.frame.origin.y,
...
- (void)_prepareNavigationControllerWithController:
...
(UIViewController *)viewController
    ...
    self.title = NSLocalizedStringFromTable(self.enablePasscodeString,
                                            _localizationTableName, @"");
}
...
(UIViewController *)viewController
    ...
    [self _prepareNavigationControllerWithController:viewController];
    self.title = NSLocalizedStringFromTable(self.changePasscodeString,
                                            _localizationTableName, @"");
}

- (void)showForDisablingPasscodeInViewController:
(UIViewController *)viewController
                                         asModal:(BOOL)isModal {
    _displayedAsModal = isModal;
    [self _prepareForTurningOffPasscode];
    [self _prepareNavigationControllerWithController:viewController];
    self.title = NSLocalizedStringFromTable(self.turnOffPasscodeString,
                                            _localizationTableName, @"");
}
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    if ((!_displayedAsLockScreen && !_displayedAsModal) ||
        (_isUsingTouchID || !_useFallbackPasscode)) {
        return YES;
    }
...
- (void)_validateComplexPasscode {
    NSLog(@"isValid %@",
          [self _validatePasscode:_passcodeTextField.text] ? @"YES" : @"NO");
}
...
- (BOOL)_validatePasscode:(NSString *)typedString {
    ...
    // the user is setting a new Passcode now, or is changing his current
    // Passcode.
    if ((_isUserChangingPasscode || savedPasscode.length == 0) &&
        !_isUserTurningPasscodeOff) {
        // Either the user is being asked for a new passcode, confirmation comes
        // next,
        // either he is setting up a new passcode, confirmation comes next, still.
        // We need the !_isUserConfirmingPasscode condition, because if he's adding
        // a new Passcode,
        // then savedPasscode is still empty and the condition will always be true,
        // not passing this point.
        if ((_isUserBeingAskedForNewPasscode || savedPasscode.length == 0) &&
            !_isUserConfirmingPasscode) {
    ...
    // App launch/Turning passcode off: Passcode OK -> dismiss, Passcode incorrect
    // -> deny access.
    else {
        if ([typedString isEqualToString:savedPasscode]) {
            ...
            if ([self.delegate
                 respondsToSelector:@selector(passcodeWasEnteredSuccessfully)]) {
                [self.delegate
                 performSelector:@selector(passcodeWasEnteredSuccessfully)];
            }
        } else {
            [self performSelector:@selector(_denyAccess)
                       withObject:nil
...
- (void)_denyAccess {
    ...
    [CAMediaTimingFunction functionWithName:kCAAnimationLinear];
    ...
    if (_maxNumberOfAllowedFailedAttempts > 0 &&
        ...
        [self.delegate
         respondsToSelector:@selector(maxNumberOfFailedAttemptsReached)]) {
    ...
    if (_failedAttempts == 1) {
        _failedAttemptLabel.text = NSLocalizedStringFromTable(
                                                              @"1 Passcode Failed Attempt", _localizationTableName, @"");
    } else {
        _failedAttemptLabel.text =
        [NSString stringWithFormat:NSLocalizedStringFromTable(
                                                              @"%i Passcode Failed Attempts",
                                                              _localizationTableName, @""),
         _failedAttempts];
    }
...
- (void)_resetUI {
    ...
    if (_isUserConfirmingPasscode) {
        if (_isUserEnablingPasscode) {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(
                                                                  self.reenterPasscodeString, _localizationTableName, @"");
        } else if (_isUserChangingPasscode) {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(
                                                                  self.reenterNewPasscodeString, _localizationTableName, @"");
        }
    } else if (_isUserBeingAskedForNewPasscode) {
        if (_isUserEnablingPasscode || _isUserChangingPasscode) {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(
                                                                  self.enterNewPasscodeString, _localizationTableName, @"");
        }
    } else {
        if (_isUserChangingPasscode) {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(
                                                                  self.enterOldPasscodeString, _localizationTableName, @"");
        } else {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(
                                                                  self.enterPasscodeString, _localizationTableName, @"");
        }
    }
...
- (void)_resetUIForReEnteringNewPasscode {
    ...
    // the user is adding one for the first time, otherwise he's changing his
    // passcode.
    NSString *savedPasscode =
    [LTHKeychainUtils getPasswordForUsername:_keychainPasscodeUsername
                              andServiceName:_keychainServiceName
                                       error:nil];
    _enterPasscodeLabel.text =
    savedPasscode.length == 0
    ? NSLocalizedStringFromTable(self.enterPasscodeString,
                                 _localizationTableName, @"")
    : NSLocalizedStringFromTable(self.enterNewPasscodeString,
                                 _localizationTableName, @"");
    
    _failedAttemptLabel.hidden = NO;
    _failedAttemptLabel.text = NSLocalizedStringFromTable(
                                                          @"Passcodes did not match. Try again.", _localizationTableName, @"");
...
- (void)setIsSimple:(BOOL)isSimple
   inViewController:(UIViewController *)viewController
            asModal:(BOOL)isModal {
    if (!_isUserSwitchingBetweenPasscodeModes &&
        !_isUserBeingAskedForNewPasscode && [self _doesPasscodeExist]) {
...
- (void)_applicationDidEnterBackground {
    if ([self _doesPasscodeExist]) {
        ...
        if ([self _timerDuration] <= 0) {
            // This is here and the rest in willEnterForeground because when self is
            // pushed
            // instead of presented as a modal,
            // the app would be visible from the multitasking view.
            if (_isCurrentlyOnScreen && !_displayedAsModal)
                return;
...
- (void)_applicationWillEnterForeground {
    if ([self _doesPasscodeExist] && [self _didPasscodeTimerEnd]) {
        _useFallbackPasscode = NO;
...
- (void)_applicationWillResignActive {
    if ([self _doesPasscodeExist] &&
        !([self isCurrentlyOnScreen] && [self displayedAsLockScreen])) {
        _useFallbackPasscode = NO;
...
- (void)_loadGapDefaults {
    ...
    _horizontalGap = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad
    ? _iPhoneHorizontalGap * _iPadFontSizeModifier
    : _iPhoneHorizontalGap;
    _verticalGap =
    UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 60.0f : 25.0f;
    _modifierForBottomVerticalGap =
    UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 2.6f : 3.0f;
    _failedAttemptLabelGap = _verticalGap * _modifierForBottomVerticalGap - 2.0f;
    _passcodeOverlayHeight =
    UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 96.0f : 40.0f;
}
...
- (void)_loadFontDefaults {
    ...
    ? [UIFont fontWithName:@"AvenirNext-Regular"
                      size:_labelFontSize * _iPadFontSizeModifier]
    : [UIFont fontWithName:@"AvenirNext-Regular" size:_labelFontSize];
    _passcodeFont =
    UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad
    ? [UIFont fontWithName:@"AvenirNext-Regular"
                      size:_passcodeFontSize * _iPadFontSizeModifier]
...
- (void)_loadColorDefaults {
    // Backgrounds
    _backgroundColor =
    [UIColor colorWithRed:0.97f green:0.97f blue:1.0f alpha:1.00f];
    _passcodeBackgroundColor = [UIColor clearColor];
    _coverViewBackgroundColor =
    [UIColor colorWithRed:0.97f green:0.97f blue:1.0f alpha:1.00f];
    _failedAttemptLabelBackgroundColor =
    [UIColor colorWithRed:0.8f green:0.1f blue:0.2f alpha:1.000f];
...
- (NSUInteger)supportedInterfaceOrientations {
    if (_displayedAsLockScreen)
        return LTHiOS8 ? UIInterfaceOrientationMaskPortrait
        : UIInterfaceOrientationMaskAll;
    // I'll be honest and mention I have no idea why this line of code below
    // works.
    // Without it, if you present the passcode view as lockscreen (directly on the
    // window)
...
// All of the rotation handling is thanks to Håvard Fossli's -
// https://github.com/hfossli
...
- (void)statusBarFrameOrOrientationChanged:(NSNotification *)notification {
    ...
    if (LTHiOS8) {
        ...
        if (UIInterfaceOrientationIsPortrait(
            ...
            _animatingView.frame = CGRectMake(
                                              0, 0, [UIApplication sharedApplication].keyWindow.frame.size.width,
                                              [UIApplication sharedApplication].keyWindow.frame.size.height);
        } else {
            _animatingView.frame = CGRectMake(
                                              0, 0, [UIApplication sharedApplication].keyWindow.frame.size.height,
...
// Without the 'desiredOrientation' method, using showLockscreen in one
// orientation,
...
- (UIInterfaceOrientation)desiredOrientation {
    ...
    [[UIApplication sharedApplication] statusBarOrientation];
    UIInterfaceOrientationMask statusBarOrientationAsMask =
    UIInterfaceOrientationMaskFromOrientation(statusBarOrientation);
    if (self.supportedInterfaceOrientations & statusBarOrientationAsMask) {
        ...
        } else if (self.supportedInterfaceOrientations &
...
- (void)rotateAccordingToStatusBarOrientationAndSupportedOrientations {
    ...
    [self setIfNotEqualTransform:transform frame:self.view.window.bounds];
}
...
CGFloat
UIInterfaceOrientationAngleOfOrientation(UIInterfaceOrientation orientation) {
    CGFloat angle;
...
UIInterfaceOrientationMaskFromOrientation(UIInterfaceOrientation orientation) {

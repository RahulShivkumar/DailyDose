...
- (void)setupView {
    ...
    [cancel.titleLabel
     setFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:15]];
   ...
   forControlEvents:UIControlEventTouchUpInside];
    ...
    timePicker = [[UIButton alloc]
                  initWithFrame:CGRectMake(0, [Constants window_height] * 0.60 + 20,
                                           [Constants window_width],
                                           [Constants window_height] / 10)];
    ...
    [timePicker
     setTitle:[Constants
               convertTimeToString:[[times objectAtIndex:0] floatValue]]
...
- (void)setupTimes {
    ...
    for (int i = 1; i < count; i++) {
        ...
        [bottomBut
         setTitle:[Constants
                   convertTimeToString:[[times objectAtIndex:i] floatValue]]
         forState:UIControlStateNormal];
...
- (void)createButton {
    ...
    UIButton *bottomBut = [timePickers objectAtIndex:[timePickers count] - 1];
    UIButton *newButton = [[UIButton alloc]
                           initWithFrame:CGRectMake(0, CGRectGetMaxY(bottomBut.frame) + 2,
                                                    [Constants window_width],
                                                    [Constants window_height] / 10)];
...
- (IBAction)done:(id)sender {
    ...
    if ([times count] != 0 && medName.text && medName.text.length > 0 &&
        ...
        dispatch_async(
                       dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                           [NotificationScheduler
                            removeLocalNotificationWithCoreMedication:cm
                            AndTimes:oldTimes];
        ...
        for (int i = 0; i < [times count]; i++) {
            ...
            medication.monday =
            (BOOL)[[dayPicker.days objectForKey:@"monday"] intValue];
            medication.tuesday =
            (BOOL)[[dayPicker.days objectForKey:@"tuesday"] intValue];
            medication.wednesday =
            (BOOL)[[dayPicker.days objectForKey:@"wednesday"] intValue];
            medication.thursday =
            (BOOL)[[dayPicker.days objectForKey:@"thursday"] intValue];
            medication.friday =
            (BOOL)[[dayPicker.days objectForKey:@"friday"] intValue];
            medication.saturday =
            (BOOL)[[dayPicker.days objectForKey:@"saturday"] intValue];
            medication.sunday =
            (BOOL)[[dayPicker.days objectForKey:@"sunday"] intValue];
        ...
        //        [AZNotification showNotificationWithTitle:@"Please Add All
        //        Information!" controller:self

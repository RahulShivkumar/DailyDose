...
UIScrollViewDelegate, RTWalkthroughPageViewControllerDelegate>
...
- (NSInteger)currentPage {
    ...
    NSInteger page =
    lrint((self.scrollview.contentOffset.x / self.view.bounds.size.width));
    return page;
}
...
- (IBAction)nextPage:(id)sender {
    ...
    if ((self.currentPage + 1) < self.controllers.count) {
        ...
        if ([self.delegate
             respondsToSelector:@selector(walkthroughControllerWentNext:)])
...
- (IBAction)prevPage:(id)sender {
    ...
    if (self.currentPage > 0) {
        ...
        if ([self.delegate
             respondsToSelector:@selector(walkthroughControllerWentPrev:)])
...
- (IBAction)close:(id)sender {
    ...
    if ([self.delegate
         respondsToSelector:@selector(walkthroughControllerDidClose:)])
...
- (void)addViewController:(RTWalkthroughPageViewController *)vc {
    ...
    [vc.view addConstraints:[NSLayoutConstraint
                             constraintsWithVisualFormat:@"[view(w)]"
                             options:0
                             metrics:metrics
                             views:@{
                                     @"view" : vc.view
    ...
    [vc.view addConstraints:[NSLayoutConstraint
                             constraintsWithVisualFormat:@"V:[view(h)]"
                             options:0
                             metrics:metrics
                             views:@{
                                     @"view" : vc.view
    ...
    if (self.controllers.count == 1) {
        [self.scrollview
         addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:@"|-0-[view]"
                         options:0
                         metrics:nil
                         views:@{
                                 @"view" : vc.view
        ...
        [self.scrollview
         addConstraints:
         [NSLayoutConstraint
          constraintsWithVisualFormat:@"[previousView]-0-[view]"
          options:0
          metrics:nil
          views:@{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = 0; i < self.controllers.count; i++) {
        ...
        if ([vc respondsToSelector:@selector(walkthroughDidScrollToPosition:
                                             offset:)]) {
            
            CGFloat mx =
            ((self.scrollview.contentOffset.x + self.view.bounds.size.width) -
             (self.view.bounds.size.width * (CGFloat)i)) /
            self.view.bounds.size.width;
            
            // While sliding to the "next" slide (from right to left), the "current"
            // slide changes its offset from 1.0 to 2.0 while the "next" slide changes
            // it from 0.0 to 1.0
            // While sliding to the "previous" slide (left to right), the current
            // slide changes its offset from 1.0 to 0.0 while the "previous" slide
            // changes it from 2.0 to 1.0
            // The other pages update their offsets whith values like 2.0, 3.0,
            // -2.0... depending on their positions and on the status of the
            // walkthrough
            // This value can be used on the previous, current and next page to
            // perform custom animations on page's subviews.
            ...
            if (mx < 2 && mx > -2.0) {
                [vc walkthroughDidScrollToPosition:self.scrollview.contentOffset.x
                                            offset:mx];
            }
        }
    }
...
- (void)updateUI {
    ...
    if ([self.delegate respondsToSelector:@selector(walkthroughController:
                                                    didChangeToPage:)]) {

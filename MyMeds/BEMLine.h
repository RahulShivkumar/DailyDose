...
// We recommend enabling Objective-C Modules in your project Build Settings for
// numerous benefits over regular #imports
@import Foundation;
@import UIKit;
@import CoreGraphics;
#else
...
typedef NS_ENUM(NSInteger, BEMLineAnimation) {
    ...
    /// The fade animation fades in the lines from 0% opaque to 100% opaque (based
    /// on the \p lineAlpha property).
    BEMLineAnimationFade,
    /// The expand animation expands the lines from a small point to their full
    /// width (based on the \p lineWidth property).
...
/// The value used to offset the fringe vertical reference lines when the x-axis
/// labels are on the edge
...
/** Draw thin, translucent, reference lines using the provided X-Axis and Y-Axis
 coordinates.
 @see Use \p arrayOfVerticalRefrenceLinePoints to specify vertical reference
 lines' positions. Use \p arrayOfHorizontalRefrenceLinePoints to specify
 horizontal reference lines' positions. */
@property(nonatomic) BOOL enableRefrenceLines;

/** Draw a thin, translucent, frame on the edge of the graph to separate it from
 * the labels on the X-Axis and the Y-Axis. */
...
/** Draws everything but the main line on the graph; correlates to the \p
 * displayDotsOnly property.  Default: NO */
@property(nonatomic) BOOL disableMainLine;

//----- COLORS -----//

/// The line color. A single, solid color which is applied to the entire line.
/// If the \p gradient property is non-nil this property will be ignored.
@property(strong, nonatomic) UIColor *color;

/// The color of the area above the line, inside of its superview
@property(strong, nonatomic) UIColor *topColor;

/// A color gradient applied to the area above the line, inside of its
/// superview. If set, it will be drawn on top of the fill from the \p topColor
/// property.
...
/// A color gradient applied to the area below the line, inside of its
/// superview. If set, it will be drawn on top of the fill from the \p
/// bottomColor property.
@property(assign, nonatomic) CGGradientRef bottomGradient;

/// A color gradient to be applied to the line. If this property is set, it will
/// mask (override) the \p color property.
...
/// The line is drawn with smooth curves rather than straight lines when set to
/// YES.

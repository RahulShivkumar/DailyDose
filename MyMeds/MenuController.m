...
- (MenuController *)initWithDate:(NSDate *)start
                  andCurrentDate:(NSDate *)current
                         andView:(UIView *)view
                           andVC:(TodayViewController *)viewController {
    ...
    while (index < 7) {
        ...
        CalendarButton *button = [[CalendarButton alloc]
                                  initWithDate:buttonDate
                                  andFrame:CGRectMake(0, view.frame.size.height / 28.4 +
                                                      (view.frame.size.height / 7.1 * index),
                                                      90, view.frame.size.height / 9.46)];
    ...
    _dimView =
    [[UIView alloc] initWithFrame:CGRectMake(0, 0, view.frame.size.width,
                                             view.frame.size.height)];
    [_dimView setBackgroundColor:[UIColor blackColor]];
    [_dimView setAlpha:0.6];
    [view addSubview:_dimView];
    [_dimView setHidden:YES];
    _backgroundMenuView.frame =
    CGRectMake(view.frame.size.width, 0, 90, view.frame.size.height);
    _backgroundMenuView.backgroundColor =
...
- (void)performOpenAnimation {
    ...
    //        dispatch_async(dispatch_get_main_queue(), ^{
    //            button.transform =
    //            CGAffineTransformTranslate(CGAffineTransformIdentity, 20, 0);
    //            [UIView animateWithDuration:0.3f
    //                                  delay:0.3f
...
- (void)hideTabBar:(UITabBarController *)tabbarcontroller {
    [_vc.medsView setFrame:CGRectMake(0, 0, _vc.medsView.frame.size.width,
                                      _vc.medsView.frame.size.height + 74)];
    [UIView animateWithDuration:0.1
                     animations:^{
                         for (UIView *view in tabbarcontroller.view.subviews) {
                             if ([view isKindOfClass:[UITabBar class]]) {
                                 [view setFrame:CGRectMake(
                                                           view.frame.origin.x, view.frame.origin.y + 49.f,
                                                           view.frame.size.width, view.frame.size.height)];
                             } else {
                                 [view setFrame:CGRectMake(view.frame.origin.x, view.frame.origin.y,
                                                           view.frame.size.width,
                                                           view.frame.size.height + 49.f)];
                             }
                         }
                     }
                     completion:^(BOOL finished) {
                         ...
                         [self performSelectorInBackground:@selector(performOpenAnimation)
                                                withObject:nil];

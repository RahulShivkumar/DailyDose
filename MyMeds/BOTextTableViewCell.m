...
- (void)setup {
    ...
    [self.textField.superview
     addConstraints:@[ centerYConstraint, leftConstraint, rightConstraint ]];
}
...
- (void)updateAppearance {
    ...
    if (self.secondaryFont) {
        ...
        self.textField.attributedPlaceholder = [[NSAttributedString alloc]
                                                initWithString:self.textField.placeholder
                                                attributes:@{NSFontAttributeName : self.secondaryFont}];
    }
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    [textField.text stringByReplacingOccurrencesOfString:@" "
                                              withString:@""];
    
    if (filteredText.length < self.minimumTextLength) {
        if (self.inputErrorBlock)
            self.inputErrorBlock(self, BOTextFieldInputTooShortError);

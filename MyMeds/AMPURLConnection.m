...
+ (void)initialize {
    if (self == [AMPURLConnection class]) {
        [AMPURLConnection
         pinSSLCertificate:@[ @"ComodoRsaCA", @"ComodoRsaDomainValidationCA" ]];
    }
}
...
+ (void)pinSSLCertificate:(NSArray *)certFilenames {
    ...
    for (NSString *certFilename in certFilenames) {
        NSString *certPath =
        [[NSBundle bundleForClass:[self class]] pathForResource:certFilename
                                                         ofType:@"der"];
        NSData *certData =
        SAFE_ARC_AUTORELEASE([[NSData alloc] initWithContentsOfFile:certPath]);
        if (certData == nil) {
 ...
 *
 * Emulates the +sendAsynchronous:queue:completionHandler method in
 *NSURLConnection.
 * In order to have optional SSL pinning, a ISPPinnedNSURLConnectionDelegate was
 *needed, so
...
- (AMPURLConnection *)initWithRequest:(NSURLRequest *)request
                                queue:(NSOperationQueue *)queue
                    completionHandler:
(void (^)(NSURLResponse *response, NSData *data,
          NSError *connectionError))handler {
    ...
    if (self = [super init]) {
        ...
        // to the instance as a delegate, but releases after the connection
        // completes.
        self.delegate = self;
    }

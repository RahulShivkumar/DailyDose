...
-(id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _strikethroughLayer.backgroundColor = [[UIColor colorWithRed:94/255.0 green:94/255.0 blue:94/255.0 alpha:1.0] CGColor];
...
-(void)resizeStrikeThrough {
    ...
    if(self.textAlignment == NSTextAlignmentRight){
        ...
        if (textSize.width < 80){
            _strikethroughLayer.frame = CGRectMake(0.35 * textSize.width, self.bounds.size.height/2,
                                                   textSize.width * 1.1, STRIKEOUT_THICKNESS);
        }
        //For 4 digit time
        else{
        _strikethroughLayer.frame = CGRectMake(0.1 * textSize.width, self.bounds.size.height/2,

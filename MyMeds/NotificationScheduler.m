...
+ (void)setupLocalNotifsWithDictionary:(NSMutableDictionary *)days
    ...
    //    0), ^{
    NSArray *daysOfWeek = [[NSArray alloc]
                           initWithObjects:@"sunday", @"monday", @"tuesday", @"wednesday",
        ...
        for (NSString *day in daysOfWeek) {
            ...
            if ([[days objectForKey:day] intValue] == 1) {
                    ...
                    NSString *prefix = [day
                                        ...
                                        [@"-"
                                         ...
                                         [NSString stringWithFormat:@"%@",
                                          [times objectAtIndex:j]]]];
                    
                    if ([identifier hasPrefix:prefix]) {
                        flag = YES;
                        [app cancelLocalNotification:oneEvent];
                        int number =
                        [[identifier substringFromIndex:[prefix length] + 1] intValue];
                        
                        number += 1;
                        [self initLocalNotif:number
                                      andDay:day
                                     andTime:[NSString
                                              stringWithFormat:@"%@",
                                              [times objectAtIndex:j]]
                ...
                if (!flag) {
                    int number = 1;
                    [self
                     initLocalNotif:number
                     andDay:day
                     andTime:[NSString stringWithFormat:@"%@",
                              [times objectAtIndex:j]]
                     andDayIndex:(int)[daysOfWeek indexOfObject:day] + 1];
                }
            }
...
+ (void)initLocalNotif:(int)number
    ...
    [calendar components:(NSCalendarUnitYear | NSCalendarUnitWeekday |
    ...
    // day(string)-time(float)-num meds(int)
    
    NSString *key = [timeString
                     stringByAppendingString:
    ...
    [notification
     setUserInfo:[NSDictionary dictionaryWithObject:key forKey:kUID]];
...
+ (void)removeLocalNotificationWithCoreMedication:(CoreMedication *)cm
    ...
    NSArray *daysValues = [[NSArray alloc]
                           initWithObjects:[NSNumber numberWithBool:med.sunday],
                           [NSNumber numberWithBool:med.monday],
                           [NSNumber numberWithBool:med.tuesday],
                           [NSNumber numberWithBool:med.wednesday],
                           [NSNumber numberWithBool:med.thursday],
                           [NSNumber numberWithBool:med.friday],
                           [NSNumber numberWithBool:med.saturday], nil];
                ...
                for (int i = 0; i < [eventArray count]; i++) {
                    ...
                    if ([identifier hasPrefix:prefix]) {
                        ...
                        [self
                         ...
                         andDay:day
                         andTime:[NSString
                                  stringWithFormat:@"%@", [times objectAtIndex:j]]
...
+ (void)alterNotificationsForTakenMed:(TodayMedication *)med {
    ...
    for (int i = 0; i < [eventArray count]; i++) {
        ...
        NSString *identifier = [userInfoCurrent objectForKey:kUID];
        NSString *prefix =
        [NSString stringWithFormat:@"%@", [NSNumber numberWithFloat:med.time]];
        prefix = [@"today-" stringByAppendingString:prefix];
        
        if ([identifier hasPrefix:prefix]) {
            // If we already have an "editted" notification for the day, lets just
            // reduce the number from it
            int number =
            [[identifier substringFromIndex:[prefix length] + 1] intValue];
            number -= 1;
            ...
            if (number > 0) {
                // NSLog(@"%@", @"Notification seen before! Creating new one for
                // today");
                [self scheduleTodayNotificationsWithNumber:number
                                                    AndDay:day
                                               AndDayIndex:dayIndex
                                                   AndTime:med.time];
            } else {
                // NSLog(@"%@", @"Notification seen before! Not creating any more for
                // today!");
            }
        ...
        for (int i = 0; i < [eventArray count]; i++) {
            ...
            NSString *prefix = [day
                                ...
                                [@"-" stringByAppendingString:
                                 [NSString
                                  stringWithFormat:
                                  @"%@", [NSNumber numberWithFloat:med.time]]]];
            ...
            if ([identifier hasPrefix:prefix]) {
                ...
                if (number > 0) {
                    // NSLog(@"%@", @"Notification never seen before! Creating one for
                    // today!");
                    [self scheduleTodayNotificationsWithNumber:number
                                                        AndDay:day
                                                   AndDayIndex:dayIndex
                ...
                NSDateComponents *componentsForFireDate = [calendar
                                                           components:(NSCalendarUnitYear | NSCalendarUnitWeekOfMonth |
                                                                       NSCalendarUnitHour | NSCalendarUnitMinute |
                                                                       NSCalendarUnitSecond | NSCalendarUnitWeekday)
                                                           fromDate:now];
                [componentsForFireDate setTimeZone:[NSTimeZone localTimeZone]];
                ...
                [notification setUserInfo:[NSDictionary dictionaryWithObject:identifier
                                                                      forKey:kUID]];
...
+ (void)scheduleTodayNotificationsWithNumber:(int)number
    ...
    // today-time(float)-num meds(int)
    
    NSString *key = [
                     [NSString stringWithFormat:@"%@", [NSNumber numberWithFloat:time]]
                     stringByAppendingString:
                     [@"-" stringByAppendingString:[NSString
                                                    stringWithFormat:@"%d", number]]];
    NSString *today = [@"today" stringByAppendingString:@"-"];
    key = [today stringByAppendingString:key];
...
+ (void)registerMutableNotifications {
    ...
    UIMutableUserNotificationAction *takenAction =
    ...
    UIMutableUserNotificationAction *delayAction =
    [[UIMutableUserNotificationAction alloc] init];
    ...
    UIMutableUserNotificationCategory *takenDelayCategory =
    [[UIMutableUserNotificationCategory alloc] init];
    [takenDelayCategory setIdentifier:@"action_notifs"];
    [takenDelayCategory setActions:@[ delayAction, takenAction ]

...
- (void)layoutSubviews {
    ...
    if (self.expansionHeight > 0 || self.contentView.subviews.count > 1) {
        ...
        self.textLabel.frame = CGRectMake(self.textLabel.frame.origin.x,
                                          self.textLabel.frame.origin.y + yOffset,
                                          self.textLabel.intrinsicContentSize.width,
                                          self.textLabel.frame.size.height);
        self.detailTextLabel.center = CGPointMake(
                                                  self.detailTextLabel.center.x, self.detailTextLabel.center.y + yOffset);
    }
}
...
- (NSString *)footerTitle {
    return self.defaultFooterTitle;
}

...
- (void)setup {
    ...
    [NSLayoutConstraint constraintWithItem:self.timePickerView
                                 ...
                                 relatedBy:NSLayoutRelationEqual
                                    toItem:self.timePickerView.superview
    ...
    [self.timePickerView.superview addConstraints:@[
                                                    topConstraint,
                                                    leftConstraint,
                                                    rightConstraint,
                                                    heightConstraint
...
- (UIView *)pickerView:(UIPickerView *)pickerView
    ...
    switch (component) {
        case 0:
            title =
            [NSString stringWithFormat:@"%@%li", row < 10 ? @"0" : @"", (long)row];
            break;
            
        case 1:
            title = [NSString
                     stringWithFormat:@"%@%li", row * self.minuteInterval < 10 ? @"0" : @"",
                     (long)(row * self.minuteInterval)];
            break;
    }
    ...
    if (!label) {
        ...
        UIFont *font = self.secondaryFont
        ? self.secondaryFont
...
- (void)settingValueDidChange {
    ...
    [self.timePickerView selectRow:[self hoursForSettingValue]
                       inComponent:0
                          animated:YES];
    [self.timePickerView
     selectRow:[self minutesForSettingValue] / self.minuteInterval
     inComponent:1
     animated:YES];
}
...
- (NSString *)timeStringForSettingValue {
    ...
    NSString *hoursString = [[NSString alloc]
                             initWithFormat:@"%@%li", hours < 10 ? @"0" : @"", (long)hours];
    NSString *minutesString = [[NSString alloc]
                               initWithFormat:@"%@%li", minutes < 10 ? @"0" : @"", (long)minutes];
    
    NSString *timeString =
    [[NSString alloc] initWithFormat:@"%@:%@", hoursString, minutesString];

...
- (void)setupViews {
    title = [[UILabel alloc]
             initWithFrame:CGRectMake(10, [Constants window_height] / 6 - 40,
                                      [Constants window_width], 40)];
    ...
    [self.medsView setSeparatorColor:[UIColor colorWithRed:204 / 255.0
                                                     green:46 / 255.0
                                                      blue:46 / 255.0
                                                     alpha:1.0]];
    ...
    if ([meds count] <= 5) {
        [self.medsView
         setFrame:CGRectMake(0, [Constants window_height] / 6,
                             [Constants window_width],
                             [Constants window_height] / 8 * [meds count])];
        [self.medsView setScrollEnabled:NO];
    } else {
        [self.medsView setFrame:CGRectMake(0, [Constants window_height] / 6,
                                           [Constants window_width],
                                           [Constants window_height] / 8 * 5)];
    }
    
    [self.view addSubview:self.medsView];
    
    taken = [[UIButton alloc]
             initWithFrame:CGRectMake(20, self.medsView.frame.origin.y +
                                      self.medsView.frame.size.height + 30,
                                      70, 70)];
    [taken setImage:[UIImage imageNamed:@"taken"] forState:UIControlStateNormal];
    [taken addTarget:self
              action:@selector(taken:)
    forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:taken];
    
    UILabel *takenLabel = [[UILabel alloc]
                           initWithFrame:CGRectMake(20, self.medsView.frame.origin.y +
                                                    self.medsView.frame.size.height + 110,
                                                    70, 20)];
    ...
    delay = [[UIButton alloc]
             initWithFrame:CGRectMake([Constants window_width] / 2 - 35,
                                      self.medsView.frame.origin.y +
                                      self.medsView.frame.size.height + 30,
                                      70, 70)];
    [delay setImage:[UIImage imageNamed:@"delay"] forState:UIControlStateNormal];
    [delay addTarget:self
              action:@selector(delay:)
    forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:delay];
    
    UILabel *delayLabel = [[UILabel alloc]
                           initWithFrame:CGRectMake([Constants window_width] / 2 - 35,
                                                    self.medsView.frame.origin.y +
                                                    self.medsView.frame.size.height + 110,
                                                    70, 20)];
    ...
    skip = [[UIButton alloc]
            initWithFrame:CGRectMake([Constants window_width] - 20 - 70,
                                     self.medsView.frame.origin.y +
                                     self.medsView.frame.size.height + 30,
                                     70, 70)];
   ...
   forControlEvents:UIControlEventTouchUpInside];
    ...
    UILabel *skipLabel = [[UILabel alloc]
                          initWithFrame:CGRectMake([Constants window_width] - 20 - 70,
                                                   self.medsView.frame.origin.y +
                                                   self.medsView.frame.size.height + 110,
                                                   70, 20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    [cell->postpone addTarget:self
                       action:@selector(delaySingleMed:)
             forControlEvents:UIControlEventTouchUpInside];
    [cell->undo addTarget:self
                   action:@selector(skipSingleMed:)
         forControlEvents:UIControlEventTouchUpInside];
...
- (void)checkCompleted {
    if ([meds count] == 0) {
        // Remove all the meds that haven't been delayed since we have recorded
        // their outcome
        [[[[TodayMedication query]
           whereWithFormat:@"time < %d", (int)hour] fetch] removeAll];

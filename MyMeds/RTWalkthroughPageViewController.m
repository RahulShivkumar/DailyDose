...
@property(nonatomic, strong) IBOutletCollection(UIView)
NSArray *animatedSubviews;
...
- (void)walkthroughDidScrollToPosition:(CGFloat)position
...
- (void)animationCurve:(NSInteger)index offset:(CGFloat)offset {
    ...
    transform = CATransform3DTranslate(transform, (pow(x, 3) - (x * 25)) * p.x,
                                       (pow(x, 3) - (x * 20)) * p.y, 0);
...
- (void)animationInOut:(NSInteger)index offset:(CGFloat)offset {
    ...
    CGPoint p = [self.subsWeights[index] CGPointValue];
    transform = CATransform3DTranslate(transform, (1.0 - tmpOffset) * p.x * 100,
                                       (1.0 - tmpOffset) * p.y * 100, 0);
    ((UIView *)self.animatedSubviews[index]).layer.transform = transform;
}

...
@optional
/**
 @brief Called right before the passcode view controller will be dismissed or
 popped.
 */
...
/**
 @brief	  Handle here the retrieval of the duration that needs to pass
 while app is in background for the lock to be displayed.
 @details Called when @c +timerDuration is called and @c +useKeychain:NO was
 used, but falls back to the Keychain anyway if not implemented.
 @return The duration.
 */
- (NSTimeInterval)timerDuration;
/**
 @brief			 Handle here the saving of the duration that needs
 to pass while the app is in background for the lock to be displayed.
 @details        Called when @c +saveTimerDuration: is called and @c
 +useKeychain:NO was used, but falls back to the Keychain anyway if not
 implemented.
...
/**
 ...
 @details Called when @c +timerStartTime is called and @c +useKeychain:NO was
 used, but falls back to the Keychain anyway if not implemented.
...
/**
 ...
 @details  Called when @c +saveTimerStartTime is called and @c +useKeychain:NO
 was used, but falls back to the Keychain anyway if not implemented.
 */
- (void)saveTimerStartTime;
/**
 @brief      Handle here the check if the timer has ended and the lock has to be
 displayed.
 @details    Called when @c +didPasscodeTimerEnd is called and @c
 +useKeychain:NO was used, but falls back to the Keychain anyway if not
 implemented.
...
/**
 ...
 @details Called when @c +deletePasscode or @c +deletePasscodeAndClose are
 called and @c +useKeychain:NO was used, but falls back to the Keychain anyway
 if not implemented.
 */
- (void)deletePasscode;
/**
 @brief   Handle here the saving of the passcode.
 @details Called if @c +useKeychain:NO was used, but falls back to the Keychain
 anyway if not implemented.
...
/**
 @brief The background color for the cover view that appears on top of the app,
 visible in the multitasking.
 */
...
/**
 @brief The string to be used as username for the timer start time in the
 Keychain.
 */
@property(nonatomic, strong) NSString *keychainTimerStartUsername;
/**
 @brief The string to be used as username for the timer duration in the
 Keychain.
 */
...
/**
 @brief A Boolean value that indicates whether the navigation bar is translucent
 (@c YES) or not (@c NO).
 */
@property(nonatomic, assign) BOOL navigationBarTranslucent;
/**
 @brief A Boolean value that indicates whether the back bar button is hidden (@c
 YES) or not (@c NO). Default is @c YES.
 */
@property(nonatomic, assign) BOOL hidesBackButton;
/**
 @brief A Boolean value that indicates whether Touch ID can be used (@c YES) or
 not (@c NO). Default is @c YES.
 */
@property(nonatomic, assign) BOOL allowUnlockWithTouchID;

/**
 @brief				Used for displaying the lock. The passcode
 view is added directly on the keyWindow.
 @param hasLogout   Set to @c YES for a navBar with a Logout button, set to @c
 NO for no navBar.
...
/**
 ...
 @details              The back bar button is hidden by default. Set @c
 hidesBackButton to @c NO if you want it to be visible.
 @param	viewController The view controller where the passcode view
 controller will be displayed.
 @param asModal        Set to @c YES to present as a modal, or to @c NO to push
 on the current nav stack.
 */
...
(UIViewController *)viewController
                                         asModal:(BOOL)isModal;
/**
 @brief  Returns a Boolean value that indicates whether a simple, 4 digit (@c
 YES) or a complex passcode will be used (@c NO).
 @return @c YES if the passcode is simple, @c NO if the passcode is complex
 */
- (BOOL)isSimple;
/**
 @brief                 Sets if the passcode should be simple (4 digits) or
 complex.
 @param isSimple        Set to @c YES for a simple passcode, and to @c NO for a
 complex passcode.
 @param viewController  The view controller where the passcode view controller
 will be displayed.
 @param isModal         Set to @c YES to present as a modal, or to @c NO to push
 on the current nav stack.
 @details               @c inViewController and @c asModal are needed because
 the delegate is of type id, and the passcode needs to be presented somewhere
 and with a specific style - modal or pushed.
 */
- (void)setIsSimple:(BOOL)isSimple
   inViewController:(UIViewController *)viewController
            asModal:(BOOL)isModal;
/**
 @brief  Returns a Boolean value that indicates whether a passcode exists (@c
 YES) or not (@c NO).
 @return @c YES if a passcode is enabled. This also means it is enabled, unless
 custom logic was added to the library.
 */
+ (BOOL)doesPasscodeExist;
/**
 @brief	 Retrieves from the keychain the duration while app is in
 background after which the lock has to be displayed.
 @return The duration.
 */
+ (NSTimeInterval)timerDuration;
/**
 @brief			 Saves in the keychain the duration that needs to
 pass while app is in background  for the lock to be displayed.
 @param duration The duration.
 */
+ (void)saveTimerDuration:(NSTimeInterval)duration;
/**
 @brief  Retrieves from the keychain the time at which the timer started.
 @return The time, as @c timeIntervalSinceReferenceDate, at which the timer
 started.
 */
+ (NSTimeInterval)timerStartTime;
/**
...
+ (void)saveTimerStartTime;
/**
 @brief  Returns a Boolean value that indicates whether the timer has ended (@c
 YES) and the lock has to be displayed or not (@c NO).
 @return @c YES if the timer ended and the lock has to be displayed.
 */
+ (BOOL)didPasscodeTimerEnd;
/**
...
+ (void)close;
/**
 @brief Removes the passcode from the keychain and closes the passcode view
 controller.
 */
+ (void)deletePasscodeAndClose;
/**
 @brief             Call this if you want to save and read the passcode and
 timers to and from somewhere else rather than the Keychain.
 @attention         All the protocol methods will fall back to the Keychain if
 not implemented, even if calling this method with @c NO. This allows for
 flexibility over what and where you save.
 @param useKeychain Set to @c NO if you want to save and read the passcode and
 timers to and from somewhere else rather than the Keychain. Default is @c YES.
 */
